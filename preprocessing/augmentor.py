# Slightly modified From https://github.com/lambdal/lambda-deep-learning-demo/blob/master/source/augmenter/ssd_augmenter.py
import numpy as np
import cv2
...
def preprocess_for_train(image,
    ...
    if speed_mode:
        ...
        boxes = tf.concat([tf.expand_dims(y1, -1),
                           tf.expand_dims(x1, -1),
                           tf.expand_dims(y2, -1),
                           tf.expand_dims(x2, -1)], axis=1)
        boxes = boxes + [shift_y, shift_x, shift_y, shift_x]
        image = new_image
...
def preprocess_for_eval(image,
    ...
    if speed_mode:
        ...
        # # mean subtraction
        # means = np.array([_R_MEAN, _G_MEAN, _B_MEAN])/255
        #
...
def preprocess_for_export(image, resolution, mb_size):
